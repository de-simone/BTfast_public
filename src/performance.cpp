...
//-------------------------------------------------------------------------- //
/*! NetPL, Nwins, GrossProfit, GrossLoss, MaxProfit, MaxLoss,
    Max and average drawdown (cash and %)
...
{
    // Variance of Profits
    double var_trade {0.0};
    if( metrics["ntrades"] > 1 ){
        var_trade = ( metrics["net_pl2"]/metrics["ntrades"]
                        - pow(metrics["avg_trade"], 2) )
