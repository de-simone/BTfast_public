...
/*! Constructor
*/
NG2::NG2( std::string name, Instrument symbol,
...
int NG2::preliminaries( const std::deque<Event>& data1,
...
*/
void NG2::compute_entry( const std::deque<Event>& data1,
                                const std::deque<Event>& data1D,
                                const PositionHandler& position_handler,
                                std::array<Event, 2> &signals )
{
    ...
    // --------------------------    TIME FILTER    ------------------------ //
    bool FilterT { true };
    FilterT = (Tsegment == 3); // 10
    // --------------------------------------------------------------------- //

    // ---------------------------    FILTER 1    -------------------------- //
    bool Filter1_short { true };
    bool Filter1_long { true };
    Filter1_long = (HighD_[0]-OpenD_[0]) > ((HighD_[1]-OpenD_[1]) * 1); // 14
    Filter1_short = (OpenD_[0]-LowD_[0])> ((OpenD_[1]-LowD_[1]) * 1);   // 14
    // --------------------------------------------------------------------- //
...
*/
void NG2::compute_exit( const std::deque<Event>& data1,
                               const std::deque<Event>& data1D,
                               const PositionHandler& position_handler,
                               std::array<Event, 2> &signals )
{
...
void NG2::compute_signals( const PriceCollection& price_collection,
                                  const PositionHandler& position_handler,
                                  std::array<Event, 2> &signals )
{
